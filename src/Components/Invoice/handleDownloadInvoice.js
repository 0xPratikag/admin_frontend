...
// add this at bottom of handleDownloadInvoice.js

export async function handleDownloadInvoiceByInvoiceId(invoiceId, { preview = false } = {}) {
  const token = localStorage.getItem("token");
  const baseURL = import.meta.env.VITE_API_BASE_URL;

  try {
    const { data } = await axios.get(`${baseURL}/invoices/${invoiceId}/pdf-payload`, {
      headers: { Authorization: `Bearer ${token}` },
    });

    const payload = data?.data || data;
    await buildAndDownloadPDF(payload, { preview, filenameFallback: invoiceId });
  } catch (err) {
    console.error(err);
